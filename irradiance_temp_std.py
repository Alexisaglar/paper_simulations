...
def plot_seasonal_profiles(avg_irradiance, avg_temperature, seasons):
    ...
    for i, season in enumerate(seasons):
        # Plot irradiance
        axs[i].plot(pd.to_datetime(avg_irradiance.loc[season].index), avg_irradiance.loc[season]['GHI'], label='GHI', color='orange', linewidth=2)
        
        # Create a twin axis for temperature
        ax2 = axs[i].twinx()
        ax2.plot(pd.to_datetime(avg_temperature.loc[season].index), avg_temperature.loc[season]['t2m'], label='Temperature', color='blue', linewidth=2, linestyle='--')
